...
def test_extrae_observaciones_de_resumen_654391() -> None:
    pagina_1 = LectorTextoPyMuPdf().leer_paginas((ROOT / "pdfs" / "Raloe" / "654391.pdf").read_bytes())[0]
    esperado = json.loads(
...
def test_devuelve_observaciones_null_si_resumen_no_tiene_observacion() -> None:
    pagina_1 = LectorTextoPyMuPdf().leer_paginas((ROOT / "pdfs" / "Raloe" / "654340.pdf").read_bytes())[0]
    esperado = json.loads(
...
def test_extrae_observaciones_continuadas_antes_de_detalle_precios() -> None:
    paginas = LectorTextoPyMuPdf().leer_paginas((ROOT / "pdfs" / "Raloe" / "654144.pdf").read_bytes())
